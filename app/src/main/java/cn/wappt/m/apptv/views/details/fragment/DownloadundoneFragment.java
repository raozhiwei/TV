 ...
 * @author: wsq
 * @date: 2020/10/29
 * Description:下载未完成的片段
 */
public class DownloadundoneFragment extends Fragment  implements  View.OnClickListener, VideoDownloadListAdapter.OnItemClickListener{
...
/*    //获取存储
                ...
                switch (msg.what) {
                    case 1:
                        for (int i = 0; i <items.size() ; i++) {
                            if (taskItem==items.get(i)){
                                if (taskItem.getTaskState()==VideoTaskState.SUCCESS){
                                    items.remove(i);
                                    mAdapter.getBrowseRecordsBases().remove(i);
   ...
   private  VideoTaskItem taskItem;
    ...
    private void initDatas() {
        ...
        if (downloadHelperUtils!=null&&downloadHelperUtils.size()>0&& items.size()>0 && items!=null){
       ...
       System.out.println("下载的id"+pos);
        ...
        if (editorStatus) {
            ...
            mAdapter.notifyDataSetChanged();
                ...
                }*/ else if (!videoLists.get(pos).isCompleted()){
            VideoDownloadManager.getInstance().fetchDownloadItems(mInfosCallback);
            System.out.println("下载的id"+pos);
            VideoTaskItem item = videoLists.get(pos);
            ...
            for (int i = 0; i <videoLists.size() ; i++) {
               if (pos==i) {
                   continue;
               }else if (videoLists.get(i).isInterruptTask()){
                   //暂停
                   VideoDownloadManager.getInstance().pauseDownloadTask(videoLists.get(i).getUrl());
                }else if (item.isRunningTask()) {
                   //恢复下载
                   VideoDownloadManager.getInstance().resumeDownload(videoLists.get(i).getUrl());
                }
            }
        }
